...
#ifndef VEUMESH_SWIFT_H
#define VEUMESH_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="VeuMesh",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// Discovers peers within Bluetooth/AWDL range using <code>MCNearbyServiceBrowser</code>
/// and advertises using <code>MCNearbyServiceAdvertiser</code>.  Messages can be relayed
/// through intermediate peers (up to <code>maxHops</code> hops) to extend range.
SWIFT_CLASS("_TtC7VeuMesh13MeshTransport")
@interface MeshTransport : NSObject
...
@class NSData;
@class MCSession;
@interface MeshTransport (SWIFT_EXTENSION(VeuMesh)) <MCNearbyServiceAdvertiserDelegate>
...
@class NSString;
@interface MeshTransport (SWIFT_EXTENSION(VeuMesh)) <MCNearbyServiceBrowserDelegate>
...
@class NSProgress;
@class NSURL;
@interface MeshTransport (SWIFT_EXTENSION(VeuMesh)) <MCSessionDelegate>
